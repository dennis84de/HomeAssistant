...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend({
    ...
    vol.Optional(CONF_UNIT_OF_MEASUREMENT, default='Tage'): cv.string,
})
...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    def run_setup(event):
        ...
        sensor_unit = config.get(CONF_UNIT_OF_MEASUREMENT)

        add_entities([SSLCertificate(sensor_name, server_name, server_port, sensor_unit)],
                     True)
...
class SSLCertificate(Entity):
    ...
    def __init__(self, sensor_name, server_name, server_port, sensor_unit):
