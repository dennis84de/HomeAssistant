...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend({
    ...
    vol.Optional(CONF_UNIT_OF_MEASUREMENT, default='Tage'):
        vol.All(cv.string, vol.In(['Minuten', 'Stunden', 'Tage']))
})
...
class UptimeSensor(Entity):
    ...
    async def async_update(self):
        ...
        if self.unit_of_measurement == 'Tage':
            div_factor *= 24
        elif self.unit_of_measurement == 'Minuten':
            div_factor /= 60
